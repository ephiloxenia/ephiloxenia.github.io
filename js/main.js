...
function initializeFeedbackDashboard() {
    // Add event listeners to feedback reply buttons
    const replyButtons = document.querySelectorAll('.feedback-reply-btn');
    replyButtons.forEach(button => {
        button.addEventListener('click', function() {
            const feedbackItem = this.closest('.feedback-item');
            const replyForm = feedbackItem.querySelector('.feedback-reply-form');
    ...
    // Add event listeners to feedback cancel buttons
    const cancelButtons = document.querySelectorAll('.feedback-cancel-btn');
    cancelButtons.forEach(button => {
        button.addEventListener('click', function() {
            const replyForm = this.closest('.feedback-reply-form');
            replyForm.classList.add('d-none');
        });
    });
    
    // Add event listeners to feedback task buttons
    const taskButtons = document.querySelectorAll('.feedback-task-btn');
    taskButtons.forEach(button => {
        button.addEventListener('click', function() {
            const feedbackItem = this.closest('.feedback-item');
            const feedbackId = feedbackItem.dataset.feedbackId;
            const feedbackContent = feedbackItem.querySelector('.feedback-content p').textContent;
            
            // Show the create task modal
            const createTaskModal = new bootstrap.Modal(document.getElementById('createTaskModal'));
            createTaskModal.show();
            
            // Populate the form with the feedback details
            const taskTitle = document.getElementById('task-title');
            const taskDescription = document.getElementById('task-description');
            
            if (taskTitle && taskDescription) {
                // Set a default task title based on feedback content
                let title = "Handle guest feedback";
                const sentimentTags = feedbackItem.querySelectorAll('.sentiment-tag.negative');
                
                if (sentimentTags.length > 0) {
                    // Use the first negative sentiment tag as the task title
                    title = `Fix: ${sentimentTags[0].textContent.trim()}`;
                }
                
                taskTitle.value = title;
                taskDescription.value = `Based on guest feedback: "${feedbackContent.trim()}"`;
                
                // Set priority based on feedback urgency
                const prioritySelect = document.getElementById('task-priority');
                if (prioritySelect) {
                    const urgentFlag = feedbackItem.querySelector('.feedback-flag .badge.bg-danger');
                    
                    if (urgentFlag) {
                        prioritySelect.value = 'urgent';
                    } else if (sentimentTags.length > 0) {
                ...
                // Set department based on feedback content
                const departmentSelect = document.getElementById('task-department');
                if (departmentSelect) {
                    // Simple keyword-based department selection
                    const content = feedbackContent.toLowerCase();
                    
                    if (content.includes('clean') || content.includes('housekeeping') || content.includes('room') || content.includes('bed')) {
                        departmentSelect.value = 'Housekeeping';
                    } else if (content.includes('check-in') || content.includes('reservation') || content.includes('front desk')) {
                        departmentSelect.value = 'Front Desk';
                    } else if (content.includes('food') || content.includes('breakfast') || content.includes('restaurant') || content.includes('meal')) {
                        departmentSelect.value = 'Food & Beverage';
                    } else if (content.includes('air conditioning') || content.includes('ac') || content.includes('heat') || content.includes('broken') || content.includes('maintenance')) {
                        departmentSelect.value = 'Maintenance';
                    }
                }
            ...
            if (taskForm) {
                taskForm.addEventListener('submit', function(e) {
                    ...
                    // Add a badge to the feedback item to indicate a task has been created
                    const feedbackItem = document.querySelector(`.feedback-item[data-feedback-id="${feedbackId}"]`);
                    
                    if (feedbackItem && !feedbackItem.querySelector('.task-badge')) {
                        const taskBadge = document.createElement('span');
                        taskBadge.className = 'badge bg-info task-badge ms-2';
                        taskBadge.innerHTML = '<i class="bi bi-check2-square"></i> Task Created';
                        
                        const feedbackActions = feedbackItem.querySelector('.feedback-actions');
                        if (feedbackActions) {
                            feedbackActions.appendChild(taskBadge);
                        }
                    }
                });
            }
    ...
    // Initialize reply form submissions
    document.querySelectorAll('.feedback-reply-form form').forEach(form => {
        form.addEventListener('submit', function(e) {
            e.preventDefault();
            ...
            const sendEmail = this.querySelector('input[type="checkbox"]').checked;
            
            if (replyText === '') return;
            
            // In a real app, you would save the reply to your backend here
            
            // Add the response to the feedback item
            ...
            // Add the response before the actions
            const feedbackActions = feedbackItem.querySelector('.feedback-actions');
            feedbackItem.insertBefore(feedbackResponse, feedbackActions);
            
            // Add resolved class to the feedback item
            feedbackItem.classList.add('feedback-resolved');
            
            // Hide the reply form
            this.closest('.feedback-reply-form').classList.add('d-none');
            
            // Clear the textarea
            this.querySelector('textarea').value = '';
            
            // Show success notification
            let message = 'Response sent successfully!';
            if (sendEmail) {
                message += ' An email has been sent to the guest.';
            }
    ...
    // Add event listener to feedback filter dropdown
    const filterDropdownItems = document.querySelectorAll('#feedbackFilterDropdown + .dropdown-menu .dropdown-item');
    filterDropdownItems.forEach(item => {
        item.addEventListener('click', function(e) {
            ...
            // Filter the feedback items
            const feedbackItems = document.querySelectorAll('.feedback-item');
            
            feedbackItems.forEach(item => {
                if (filterValue === 'All Feedback') {
                    item.style.display = '';
                } else if (filterValue === 'Needs Response' && !item.querySelector('.feedback-response')) {
                    item.style.display = '';
                } else if (filterValue === 'Negative Feedback' && item.querySelector('.sentiment-tag.negative')) {
                    item.style.display = '';
                } else if (filterValue === 'Resolved' && item.classList.contains('feedback-resolved')) {
                    item.style.display = '';
                } else {
            ...
            const filterDropdown = document.getElementById('feedbackFilterDropdown');
            if (filterDropdown) {
                filterDropdown.innerHTML = `<i class="bi bi-funnel"></i> ${filterValue}`;
            }
        });
    });
}
